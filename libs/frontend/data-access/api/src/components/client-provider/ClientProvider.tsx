...
import { createFetchApiClient, createMockApiClient, createWSApiClient } from "../../lib/frontend-data-access-api";
...
function ClientProvider(props: ClientProviderProps) {
    ...
    const client: ClientContextType = {
        getFetchClient() {
            if (fetchClient === null)
                fetchClient = createFetchApiClient(props.url);
            return fetchClient;
        },

        getWsClient() {
            if (wsClient === null)
                wsClient = createWSApiClient(props.url);
            return wsClient;
        },
...
export function useClient() { return useContext<ClientContextType | undefined>(ClientContext) }
